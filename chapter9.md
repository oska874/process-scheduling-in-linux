...
Linux 调度器支持软实时（RT）任务调度。这意味着内核可以有效的调度有严格时间限制的任务。然而内核通常又能力满足非常严格的实时任务调度的最终截至时间，但是并不能保证一定能满足这个截止时间。
...
根据优先级的实现内容，RT 调度类也遵循之前 O(1) 调度器同样的规则。内核使用多个运行队列，每个队列对应一个优先级。按照这种方式，添加、删除或者寻找最高优先级任务的操作可以在 O(1) 时间内完成。
...
`rt_rq` 在 `kernel/sched.c` 实现。第一个低端保持了优先级数组。其他几乎所有的字段都是和 SMP 和组调度相关的。
...
`pick_next_rt_entity()` 会从运行对了中选出优先级最高的任务作为下一个运行的任务。
